...
async function loadCards() {
  ...
  for(let i = 0; i < customDir.length; i++) {
    ...
    if(curr.type === "FILE" && curr.entry.endsWith(".tar.gz")) {
      await Neutralino.os.execCommand(`tar --force-local -xzf "${NL_PATH}${S}custom${S}${curr.entry}" -C "${NL_PATH}${S}custom${S}"`);
...
async function importCustom() {
  ...
  await Neutralino.os.execCommand(`tar --force-local -xzf "${file}" -C "${NL_PATH}${S}custom${S}.tmp"`);
